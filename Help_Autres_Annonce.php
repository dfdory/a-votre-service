...
<div class="breadcrumb">
				<div>
						...
						<?php if(isset($_SESSION['statut']) && $_SESSION['statut']==1){$pag=$_SESSION['statut'];?><h2 class="title">D&eacute;posez une annonce:</h2><?php }
						elseif(isset($_SESSION['statut']) && $_SESSION['statut']==2){$pag=$_SESSION['statut'];?><h2 class="title"3>D&eacute;posez une annonce:</h2><?php }
						else{if(isset($_REQUEST['page'])&& $_REQUEST['page']==4){$pag=$_REQUEST['page']; ?><h2 class="title">D&eacute;posez une annonce:</h2>
					<?php }elseif(isset($_REQUEST['page'])&& $_REQUEST['page']==5){$pag=$_REQUEST['page'];?><h2 class="title">D&eacute;posez une annonce:</h2><?php }}?></span>
...
<div class="row">
					...
					<?php while($row = mysql_fetch_array($request)){?>                                
                        <div class="span3 element category01 height_4column" data-category="category01">
                            <div class="hover_img"><a href="Help_Autres_Annonce_suite.php?idmn=<?php echo $row['id_services'];?>&pag=<?php echo $pag;?>" >
                                <img src="images/portfolio/<?php echo $row['icon_services'];?>" alt="<?php echo $row['nom_services'];?>" /></a>
                            </div> 
                            <div class="item_description">
                              
                                <div class="descr"><a href="Help_Autres_Annonce_suite.php?idmn=<?php echo $row['id_services'];?>&pag=<?php echo $pag;?>" ><?php echo $row['nom_services'];?></a></div>
                            </div>                                    
                        </div>
						<?php }?>
